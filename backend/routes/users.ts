...
userRoute.post("/register", async (req: Request, res: Response)=>{
    try {
        const { usertype, firstName, lastName, middleName,email,username, password  } = req.body;
...
userRoute.post("/login", async (req: Request, res: Response) => {
    try {
        ...
        if (user.usertype !== 'Admin') {
            return res.status(200).json({ message: "User login.", user });
        }

        // Password is correct and account type is Admin, so proceed with login
        res.status(200).json({ message: "Admin login.", user });
    } catch (error) {
