<template>
  ...
  return !(form.firstName && form.lastName && form.email && form.contactNo && form.city && form.state &&form.password );
  });
  
  const handleSubmit = async (e) => {
  e.preventDefault();
  const { firstName, middleName, lastName, email, contactNo, city, state, password } = form;
